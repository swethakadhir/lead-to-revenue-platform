...
export function StageSettingsForm({ stage }: { stage: TableRow<"pipeline_definitions"> }) {
  ...
  return <form action={action} className="flex flex-wrap items-center gap-2"><input name="key" type="hidden" value={stage.key} /><span className="w-40 text-xs text-slate-500">{stage.key} · {stage.stage_type}</span><input aria-label={`${stage.key} stage name`} className={input} defaultValue={stage.name} name="name" required /><button className="rounded-lg border border-slate-300 px-3 py-2 text-sm disabled:opacity-60" disabled={pending}>Save</button><Notice state={state} /></form>;
}
