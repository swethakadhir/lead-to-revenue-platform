...
export async function createBusiness(_state: CreateBusinessState, formData: FormData): Promise<CreateBusinessState> {
  ...
  if (name.length < 2 || name.length > 120) return { error: "Business name must be between 2 and 120 characters." };
  if (!/^[a-z0-9]+(?:-[a-z0-9]+)*$/.test(slug) || slug.length > 63) return { error: "Slug must use lowercase letters, numbers, and single hyphens." };
  if (!timezone || timezone.length > 100) return { error: "Enter a valid IANA timezone, such as Asia/Kolkata." };
  if (!/^[A-Z]{3}$/.test(currency)) return { error: "Currency must be a three-letter ISO code, such as USD." };
