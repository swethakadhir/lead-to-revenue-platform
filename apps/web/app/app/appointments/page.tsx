...
export default async function AppointmentsPage({ searchParams }: { searchParams: SearchParams }) {
  ...
  const [appointments, references, configuration] = await Promise.all([listAppointments(tenant.id, { status, assignee, from: fromIso, to: toIso }), getOperationalReferences(tenant.id), getTenantConfiguration(tenant.id)]);
  const types = appointmentTypes(configuration.settings.appointment_types);
